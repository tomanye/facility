ALTER PROCEDURE [dbo].[SOHByProgram]
	@storeid int,
	@month int,
	@year int,
	@days int
...
BEGIN
	...
	(	select 
			...
			vw.ID, vw.FullItemName, vw.StockCode, vw.Unit, vw.Name as Type,vw.TypeID, p.ID as ProgramID, p.Name as ProgramName ,vw.DefaultUnitID
		from vwGetAllItems vw 
		inner join 
			ProgramProduct pp on vw.ID = pp.ItemID and pp.StoreID = @storeid
		left join 
			Programs p on pp.ProgramID = p.ID 
		left join 
