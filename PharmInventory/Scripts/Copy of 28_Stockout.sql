...
BEGIN
	...
	(select 
		...
		from vwGetAllItems vw left join ProgramProduct pp on vw.ID = pp.ItemID right join (select * from Programs where ID = @progID) as p on pp.ProgramID = p.ID left join 
