CREATE PROCEDURE [dbo].[SOHForStockOut]
	@storeid int,
	@month int,
	@year int,
	@days int
...
BEGIN
	...
	(select 
		...
		left join
		(select Sum(DateDiff(Day ,StartDate,EndDate)) as Quantity,ar.ItemID from Stockout ar where ar.StoreID=@storeid and StartDate  >'2013-07-07 00:00:00.000'  and EndDate < '2014-07-07 00:00:00.000' group by ItemID)as dos on dos.ItemID = vw.ID 
	     left join
